...
from prepare import (ATTEMPT, PACKAGE, PLAN, ROOT, BINARY, BUILD_INPUTS,
                     ...
                     open_exclusive, write_json, host_validation_command)
...
def main():
    ...
    try:
        ...
        steps = [retained_step('host-public-validation', host_validation_command()),
                 retained_step('device-compile', device),
        ...
        if (qualification.get('status') != 'qualified_for_selector_free_production_confirmation'
                or qualification.get('decision') != 'prepare_selector_free_whole_c1_confirmation'
                or qualification.get('production_routing_authorized') is not False
    ...
    if status != 'passed':
        return 1
    print('projected_residual_production_measure: PASS; selector-free C1 production confirmation remains required')
    return 0
