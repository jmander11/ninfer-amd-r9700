...
This fresh, create-only report-correction retry confirms the promoted public
...
```bash
bash profiles/bench/r9700-a8q4-projected-residual-t1-promoted-qualification-retry1-20260920/commands.sh --preflight
bash profiles/bench/r9700-a8q4-projected-residual-t1-promoted-qualification-retry1-20260920/commands.sh --measure
```
...
The original promoted qualification attempt passed its numerical and timing gates
but retained stale pre-promotion report disposition strings. That immutable attempt
is bound as diagnostic evidence; this retry requires the corrected active report
contract while keeping the same numerical, static, and timing criteria.
The required status is `qualified_for_selector_free_production_confirmation`,
decision is `prepare_selector_free_whole_c1_confirmation`, and
`production_routing_authorized` remains false until whole confirmation closes.
