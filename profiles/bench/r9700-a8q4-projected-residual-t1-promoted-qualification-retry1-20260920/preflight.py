...
from prepare import (ATTEMPT, PLAN, ROOT, BINARY, PACKAGE, BUILD_INPUTS,
                     SOURCE_INPUTS, SCRIPT_INPUTS, EVIDENCE_INPUTS, WHOLE_INPUTS, DIAGNOSTIC_INPUTS,
                     compile_commands, identity, require_environment,
                     verify_accepted_evidence, verify_selector_free_build, host_validation_command)

def verify():
    require_environment()
    plan = json.loads(PLAN.read_text())
    if (plan.get('schema') != 'ninfer.r9700.projected-residual-promoted-qualification-plan.v1'
    ...
    for key, paths in [('bound_inputs', SOURCE_INPUTS + SCRIPT_INPUTS),
                       ...
                       ('accepted_production_qualification', EVIDENCE_INPUTS),
                       ('accepted_whole_ab', WHOLE_INPUTS),
                       ('stale_report_diagnostic', DIAGNOSTIC_INPUTS)]:
...
def main():
    verify()
    subprocess.run(host_validation_command(), cwd=ROOT, check=True)
