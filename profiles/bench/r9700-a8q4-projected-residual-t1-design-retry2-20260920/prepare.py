#!/usr/bin/env python3
"""Bind the serialization-only retry while preserving both failed attempts."""
...
PRIOR = ROOT / "profiles/bench/r9700-a8q4-projected-residual-t1-design-retry-20260920"
...
SCRIPT_INPUTS = [PACKAGE / name for name in
                 ("commands.sh", "prepare.py", "preflight.py", "measure.py", "serializer_regression.py", "README.md")]
...
def main() -> int:
    ...
    plan["schema"] = "ninfer.r9700.a8q4-projected-residual-t1-retry2-plan.v1"
    plan["status"] = "prepared_for_review"
    plan["claim"] = "Retry the unchanged direct candidate with JSON control-character escaping repaired."
    plan["repair"] = {
        "scope": "report string serialization only; retain the prior explicit-stream repair",
        "unchanged": ["production Ops", "kernel mechanism", "oracle", "shapes", "sample counts", "thresholds", "stream ownership"],
    ...
    plan["prior_retry_evidence"] = [identity(path) for path in sorted(PRIOR.rglob("*"))
                                    if path.is_file() and "__pycache__" not in path.parts]
    if not any(Path(item["path"]).parent == PRIOR / "attempt-1"
               for item in plan["prior_retry_evidence"]):
        raise RuntimeError("first retry attempt evidence is missing")
    ...
    print(f"projected_residual_retry2_prepare: PASS {PLAN}")
    return 0
