...
def main() -> int:
    ...
    try:
        ...
        retained_step("serializer-regression",
                      regression_command(ATTEMPT / "qual", ATTEMPT), serializer_output)
        check_output(serializer_output.read_bytes())
        ...
        print(f"projected_residual_retry2_measure: FAIL {error_text}", file=sys.stderr)
    finally:
    ...
    if status != "passed":
        return 1
    print("projected_residual_retry2_measure: PASS; whole C1 A/B preparation only")
    return 0
