...
Host-only preparation binds an explicitly supplied existing production build and a fresh
linked public-Op qualification after selector removal. It never configures, builds, or runs
HIP. `--preflight` only reads and checks existing inputs; neither command repairs inputs.
`plan.json` and `attempt-1` are create-only. Review the prepared plan before `--measure`.

Run from the repository root with:

```bash
bash profiles/bench/r9700-projected-residual-t1-production-confirmation-20260920/commands.sh --prepare --build-dir /absolute/qualified/build --qualification-attempt /absolute/fresh/public-op/attempt-1
bash profiles/bench/r9700-projected-residual-t1-production-confirmation-20260920/commands.sh --preflight
bash profiles/bench/r9700-projected-residual-t1-production-confirmation-20260920/commands.sh --measure
```
