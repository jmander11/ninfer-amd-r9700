"""Check the 45 new concurrent projection kernels and exact retained instruction streams."""
import re
import subprocess
...
def inspect(binary, label):
    ...
    for note in notes.split('  - .args:')[1:]:
        ...
        instructions = [line.split('//')[0].strip() for line in body.splitlines() if line.strip() and line.strip() != '...']
...
old_receipt, old = inspect(ROOT/'profiles/bench/r9700-dflash-projections-tail-20260923/final-bin/bench', 'projection-baseline')
new_receipt, new = inspect(ROOT/'build-r9700/bench/ninfer_bench', 'projection-candidate')
for shape, original in old.items():
    assert new[shape] == original, (shape, 'retained instructions/resources changed')
added = {(n,k,t) for n,k in [(4096,5120),(12288,5120),(5120,6144),(7168,5120),(6144,5120),(1280,5120),(5120,4096),(5120,25600)] for t in [10,12,15,18,20,24]} - set(old)
...
write_json(HERE/'projection-isa.json', dict(rows=rows, retained_instruction_exact=list(old),
