"""Recheck cold Engine transitions after paired QK; no timing admission."""
import json
import os
...
trace=here/'tail-paired-decisions.json'
results=[]
for mode,stride in [('fixed3',1),('graph',1),('graph',17),('eager',1)]:
    name=f'tail-paired-{mode}-s{stride}'
    if mode=='eager': os.environ['NINFER_DFLASH_DECISION_TRACE_OUT']=str(trace)
    run([here/'tail-check-paired',artifact,root/'profiles/bench/r9700-compact-mixed-delivery-20260923/code.ids',
...
write_json(here/'tail-paired-summary.json',dict(results=results,eager_round_rows=len(rows),
