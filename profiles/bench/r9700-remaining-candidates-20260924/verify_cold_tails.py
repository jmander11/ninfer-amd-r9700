"""Recheck cold Engine transitions after selected gate/up, PV and GDN changes; no timing admission."""
import json
import os
...
trace=here/'tail-selected-decisions.json'
results=[]
for mode,stride in [('fixed3',1),('graph',1),('graph',17),('eager',1)]:
    name=f'tail-selected-{mode}-s{stride}'
    if mode=='eager': os.environ['NINFER_DFLASH_DECISION_TRACE_OUT']=str(trace)
    run([here/'tail-check-selected',artifact,root/'profiles/bench/r9700-compact-mixed-delivery-20260923/code.ids',
    ...
    ordinary=json.loads((root/'profiles/bench/r9700-dflash-projections-tail-20260923'/prior/'tail-ordinary.json').read_text())['cases']
...
write_json(here/'tail-selected-summary.json',dict(results=results,eager_round_rows=len(rows),
