...
baselines="$root/profiles/bench/r9700-a8q4-pair-wmma-c2c4-design-20260919/whole-baselines.json"
test "$PWD" = "$root"

compile_host() {
  /opt/rocm/bin/hipcc -O3 -std=c++20 --offload-arch=gfx1201 \
    -I"$root/include" -I"$root/src" -I"$root/tools/r9700" -isystem /opt/rocm/include \
    -Wall -Wextra -Werror -Wno-unused-function \
    -DNINFER_SOURCE_DIR='"/ssdpool2nvme/local_llm/ninfer-amd-r9700"' \
    "$root/tools/r9700/paired_projection_c2c4_op_qual.hip" \
    "$root/src/ops/r9700/paired_projection/paired_projection.hip" \
...
compile_device() {
  /opt/rocm/bin/hipcc -O3 -std=c++20 --offload-arch=gfx1201 \
    ...
    "$root/src/ops/r9700/paired_projection/paired_projection.hip" -o "$1"
}
...
case "${1:-}" in
  ...
  --measure)
    ...
    python3 "$root/tools/r9700/check_paired_projection_c2c4_op_static.py" \
      "$build/op.s" > "$build/static-receipt.txt"
    "$build/qual" --out-json "$report" --assembly "$build/op.s" \
      --static-receipt "$build/static-receipt.txt" --whole-baselines "$baselines"
    ;;
  *) echo "usage: $0 --preflight|--measure" >&2; exit 2 ;;
esac
