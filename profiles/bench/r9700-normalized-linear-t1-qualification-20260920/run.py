#!/usr/bin/env python3
"""Create-only, core-linked normalized-linear qualification and retained closure."""
...
def build_profile():
    ...
    expected = {'CMAKE_BUILD_TYPE': 'Release', 'CMAKE_HIP_ARCHITECTURES': 'gfx1201',
                ...
                'NINFER_R9700_NORMALIZED_LINEAR_T1_CANDIDATE': '0'}
...
def assembly_command(directory):
    ...
    for flag in ('--offload-arch=gfx1201', '-DNINFER_R9700_Q4_ACTIVATION_BITS=8',
                 '-DNINFER_R9700_NORMALIZED_LINEAR_T1_CANDIDATE=0'):
...
def current_plan():
    ...
    return {
        'schema': 'ninfer.r9700.normalized-linear-t1-plan.v1',
        'status': 'prepared_for_independent_review', 'production_routing_authorized': False,
        'claim': 'Direct public RMSNorm+Linear versus normalized_linear graph admission at 64 MLP boundaries.',
        ...
        'passing_authorizes': 'whole C1 A/B preparation only; no caller or production promotion'}
...
def measure():
    ...
    if status == 'passed':
        print('normalized_linear_measure: PASS; independent result review and whole C1 A/B remain')
