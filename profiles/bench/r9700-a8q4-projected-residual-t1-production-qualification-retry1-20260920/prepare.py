...
FAILED = ROOT / 'profiles/bench/r9700-a8q4-projected-residual-t1-production-qualification-20260920'
...
FAILED_INPUTS = [FAILED / name for name in
                 ('plan.json', 'attempt-1/closure.json', 'attempt-1/result.sha256',
                  'attempt-1/qualifier.process.json', 'attempt-1/qualifier.stderr')]
...
def main():
    ...
    failed_plan = json.loads((FAILED / 'plan.json').read_text())
    failed_closure = json.loads((FAILED / 'attempt-1/closure.json').read_text())
    failed_stderr = (FAILED / 'attempt-1/qualifier.stderr').read_text()
    if (failed_closure['status'] != 'failed'
            or failed_closure['plan'] != identity(FAILED / 'plan.json')
            or 'HIP device differs from pre-initialization power device' not in failed_stderr):
        raise RuntimeError('expected immutable power-device diagnostic failure')
    ...
    plan = {
        ...
        'retry_reason': 'Correct DRM-card-number power path to established R9700 PCI identity; no admission criterion changes',
