...
SUFFIX = 'panel-attention-20260921'
...
ATTENTION_ADMISSION = REPO / 'profiles/bench/r9700-chunked-attention-candidate-leaf-retry1-20260921/whole/result.json'
RECIPES = {
...
def build_root(group, attention):
    return REPO / f'build-r9700-selection-panel-{attention}-g{group}-20260921'
...
def inspect_inputs():
    admission = json.loads(ATTENTION_ADMISSION.read_text())
    if admission.get('status') != 'admitted':
        raise ValueError('bounded-panel attention has not passed qualification and whole admission')
    artifacts = []
    for artifact in RECIPES.values():
        artifacts.append(inspect_candidate_artifact(REPO / 'out' / f'{artifact}.ninfer'))
    paths = [Path(__file__), PACKAGE / 'commands.sh', ATTENTION_ADMISSION, RUNNER, SELECTOR,
