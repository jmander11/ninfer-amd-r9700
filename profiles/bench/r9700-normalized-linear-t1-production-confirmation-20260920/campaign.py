...
PRIOR = ROOT / 'profiles/bench/r9700-normalized-linear-t1-whole-ab-20260920'
...
SCHEMA = 'ninfer.r9700.normalized-linear-production-confirmation.v1'
...
def prior_evidence():
    ...
    common.retained_closure(direct_dir)
    ...
    candidate_median = statistics.median(r['decode_seconds'] for r in records if r['role'] == 'candidate')
    control_median = statistics.median(r['decode_seconds'] for r in records if r['role'] == 'control')
    if candidate_median != 8.735625429 or control_median != 8.826095962:
        fail('retained admission medians differ')
...
def qualified_build(directory, build):
    ...
    if (closing.get('status') != 'passed'
            or report.get('schema') != 'ninfer.r9700.normalized-linear-t1-qualification.v1'
            ...
            or summary.get('passing_authorizes') != 'selector-free whole C1 production confirmation preparation only'
    ...
    required = [build / 'CMakeCache.txt', build / 'compile_commands.json',
                build / 'src/libninfer_r9700_core.a',
                build / 'src/ninfer_r9700_a8q4_normalized_linear_t1_qual']
    for path in required:
        if inputs.get(str(path)) != identity(path): fail(f'fresh qualification does not bind build input: {path}')
    if common.SELECTOR in (build / 'compile_commands.json').read_text():
        fail('retired selector remains in compile commands')
    return qualification


def current_benchmark(build):
    # This is a dry run only: detect stale linked executable without rebuilding.
    completed = subprocess.run(['ninja', '-C', str(build), '-n', 'bench/ninfer_bench'],
                               cwd=ROOT, capture_output=True, text=True)
    if completed.returncode or completed.stderr or not completed.stdout.rstrip().endswith('ninja: no work to do.'):
        fail('production benchmark must already be fully built from current sources')
    return {'command': ['ninja', '-C', str(build), '-n', 'bench/ninfer_bench'],
...
def prepare(build, directory):
    ...
    sources += [identity(ROOT / 'tools/r9700/a8q4_normalized_linear_t1_qual.hip')]
    ...
    build_receipt = current_benchmark(build)
    preparation_telemetry = common.telemetry()
    plan = {'schema': SCHEMA, 'status': 'prepared_awaiting_independent_review',
            ...
            'preparation_build_check': build_receipt, 'preparation_telemetry': preparation_telemetry,
            'build': {'directory': str(build), 'values': cache(build),
                      ...
                      'compile_commands': identity(build / 'compile_commands.json')},
            ...
            'workload': WORKLOAD}
    exclusive(PLAN, plan)
    print('normalized_linear_production_prepare: PASS; review before measurement')


def preflight():
    if ATTEMPT.exists() or ATTEMPT.is_symlink(): fail('attempt already exists')
    plan = load(PLAN)
    if (plan.get('schema') != SCHEMA or plan.get('runs') != 3
            or plan.get('admission') != ADMISSION or plan.get('workload') != WORKLOAD):
    ...
    for key in ('cache', 'archive', 'executable', 'compile_commands'): check(plan['build'][key])
    current_benchmark(build)
    ...
    common.telemetry()
    print('normalized_linear_production_preflight: PASS')
    return plan
...
def measure():
    ...
    try:
        ...
        print('normalized_linear_production_confirmation: PASS')
    except BaseException as error:
        ...
        exclusive(ATTEMPT / 'closure.json', {
            'schema': 'ninfer.r9700.normalized-linear-production-confirmation-closure.v1',
            ...
            'create_only': True, 'selector_free_production_confirmed': failure is None})
