# Projected residual T1 production qualification retry2
...
to match it. The first retry then failed at malformed-input rejection: the public wrapper allowed
an invalid plane through to the fatal HIP-check path. This retry binds the corrected
public validation and requires the host malformed-binding qualifier to pass with
both HIP and ROCR device visibility disabled. Both prior failed packages, complete
closures, and result manifests are preserved and verified before execution.
Numerical, static, timing, and admission criteria are unchanged.
...
```bash
bash profiles/bench/r9700-a8q4-projected-residual-t1-production-qualification-retry2-20260920/commands.sh --preflight
bash profiles/bench/r9700-a8q4-projected-residual-t1-production-qualification-retry2-20260920/commands.sh --measure
```

Preflight runs host malformed-binding rejection with device visibility disabled,
then compiles source assembly and checks the bound executable's serializer/ISA
