...
SELECTOR = 'NINFER_R9700_NORMALIZED_LINEAR_T1_CANDIDATE'
FIELD = 'normalized_linear_t1_candidate'
BUILDS = {'control': ROOT / 'build-r9700-normalized-linear-whole-control-20260920',
          'candidate': ROOT / 'build-r9700-normalized-linear-whole-candidate-20260920'}
AUTHORITY = ROOT / 'profiles/bench/r9700-projected-residual-t1-production-confirmation-retry1-20260920/attempt-1/run-1-production.json'
...
SOURCES = [ROOT / name for name in (
    'CMakeLists.txt', 'src/CMakeLists.txt', 'include/ninfer/ops/normalized_linear.h',
    ...
    'src/targets/qwen3/impl/runtime/text_context.h',
    'src/targets/qwen3/impl/runtime/text_context_impl.h',
    'tests/test_r9700_fp8_execution_state_contract.cpp', 'tests/test_ninfer_bench_support.cpp',
    'tests/CMakeLists.txt', 'tools/r9700/gdn_op_qual.hip')]
...
def retained_closure(directory):
    seen = set()
...
def qualify(directory):
    ...
    correct = report.get('correctness', {})
    if (report.get('schema') != 'ninfer.r9700.normalized-linear-t1-qualification.v1'
            or report.get('status') != 'qualified_for_whole_ab_only'
            or report.get('shape') != {'tokens': 1, 'rows': 34816, 'columns': 5120}
            or correct.get('poison_stale_finite_replays') != 3
            or any(correct.get(key) is not True for key in (
                'exact_codec', 'exact_public_control', 'canaries',
                'malformed_alias_rejections', 't2_fallback', 'immutable_inputs'))
            or summary.get('status') != 'passed'
            or summary.get('saved_ms_per_token', 0) < 0.2
            or summary.get('ratio_mean_plus_2se', 1) >= 1
            or closing.get('production_routing_authorized') is not False):
    ...
    return {'attempt': str(directory), 'closure': identity(directory / 'closure.json'),
            ...
            'summary': identity(directory / 'summary.json'), 'plan': closing['plan']}
...
def prepare(directory):
    ...
    authority_summary = load(AUTHORITY.parent / 'summary.json')
    if (authority_summary.get('status') != 'passed'
            or authority_summary.get('selector_free_production_confirmed') is not True
            or authority_summary.get('exact_public_token_parity') is not True):
        fail('selector-free token authority unaccepted')
    builds = {}
    for role, build in BUILDS.items():
        ...
        targets = ['ninfer_bench', 'ninfer_r9700_fp8_execution_state_contract_test', 'ninfer_bench_support_test']
        ...
        host_tests = []
        hidden = {**os.environ, 'HIP_VISIBLE_DEVICES': '-1', 'ROCR_VISIBLE_DEVICES': '-1'}
        for name in ('ninfer_r9700_fp8_execution_state_contract_test', 'ninfer_bench_support_test'):
            executable = build / 'tests' / name
            started = time.time_ns()
            receipt = subprocess.run([str(executable)], cwd=ROOT, env=hidden,
                                     check=True, capture_output=True, text=True)
            host_tests.append({'executable': identity(executable), 'command': [str(executable)],
                               'cwd': str(ROOT), 'environment': {
                                   'HIP_VISIBLE_DEVICES': '-1', 'ROCR_VISIBLE_DEVICES': '-1'},
                               'exit_code': receipt.returncode, 'started_unix_ns': started,
                               'finished_unix_ns': time.time_ns(), 'stdout': receipt.stdout,
                               'stderr': receipt.stderr})
        builds[role] = {'cache': identity(build / 'CMakeCache.txt'), 'values': cache(build, role),
                        'executable': identity(build / 'bench/ninfer_bench'), 'host_tests': host_tests}
    ...
    exclusive(PLAN, {'schema': 'ninfer.r9700.normalized-linear-whole-ab.v1',
                    ...
                    'token_authority': identity(AUTHORITY),
                    'token_authority_manifest': identity(AUTHORITY.parent / 'result.sha256'),
                    'expected_tokens': tokens(load(AUTHORITY)),
    ...
    print('normalized_linear_whole_prepare: PASS; independent review required before measurement')


def preflight():
    if ATTEMPT.exists() or ATTEMPT.is_symlink(): fail('attempt already exists')
    plan = load(PLAN)
    if plan.get('schema') != 'ninfer.r9700.normalized-linear-whole-ab.v1' or plan.get('order') != ORDER:
        fail('plan contract differs')
    for item in plan['sources'] + [plan[k] for k in ('artifact', 'corpus', 'token_authority', 'token_authority_manifest')]: check(item)
    ...
    for role, build in plan['builds'].items():
        for key in ('cache', 'executable'): check(build[key])
        for test in build['host_tests']:
            check(test['executable'])
            if test['exit_code'] != 0: fail('GPU-hidden host test failed')
    ...
    retained_closure(AUTHORITY.parent)
    telemetry()
    print('normalized_linear_whole_preflight: PASS')
    return plan
...
def measure():
    ...
    try:
        ...
        print('normalized_linear_whole_ab: PASS')
    except BaseException as error:
        failure = f'{type(error).__name__}: {error}'
        exclusive(ATTEMPT / 'failure.json', {'status': 'failed', 'error': failure})
        raise
    finally:
        exclusive(ATTEMPT / 'closure.json', {
            'schema': 'ninfer.r9700.normalized-linear-whole-ab-closure.v1',
            'status': 'passed' if failure is None else 'failed', 'error': failure,
            'plan': identity(PLAN), 'started_unix_ns': started, 'finished_unix_ns': time.time_ns(),
