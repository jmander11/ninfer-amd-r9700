...
def validate_retry_failure(package: dict) -> None:
    helpers = package.get("imported_helpers", {})
    if set(helpers) != {"original_whole_runner"}:
        fail("imported executable helper inventory differs")
    if require_identity(helpers["original_whole_runner"], "original whole runner") != PRIOR_RUN:
        fail("imported whole runner path differs")
    retained = package.get("retained_retry_failure", {})
    if set(retained) != {"plan", "result_manifest", "failure_process", "failure_report",
                         "failure_stderr"}:
        fail("sealed retry failure inventory differs")
    paths = {label: require_identity(record, f"sealed retry {label}")
             for label, record in retained.items()}
    prior = load(paths["plan"])
    if (prior.get("schema") != "ninfer.r9700.bf16-gdn-control-t1-whole-ab-retry-plan.v1" or
            prior.get("status") != "prepared" or
            prior.get("reviewed_plan") != package["reviewed_plan"] or
            prior.get("retained_failure") != package["retained_failure"]):
        fail("sealed retry plan authority differs")
    for record in prior.get("sources", []):
        require_identity(record, "sealed retry source")
    records = {}
    for line in paths["result_manifest"].read_text().splitlines():
        checksum, name = line.split("  ", 1)
        if name in records or Path(name).name != name:
            fail("sealed retry manifest entry differs")
        records[name] = checksum
    if set(records) != {"run-1-control.json", "run-1-control.process.json",
                        "run-1-control.stdout", "run-1-control.stderr"}:
        fail("sealed retry manifest inventory differs")
    for name, checksum in records.items():
        if digest(paths["result_manifest"].parent / name) != checksum:
            fail("sealed retry result changed")
    process = load(paths["failure_process"])
    if (process.get("stem") != "run-1-control" or process.get("role") != "control" or
            process.get("exit_code") != 0 or process.get("power_after") != "auto" or
            process.get("vram_before", {}).get("passed") is not True or
            process.get("vram_after", {}).get("passed") is not False or
            process.get("benchmark_report") != identity(paths["failure_report"]) or
            process.get("stderr") != identity(paths["failure_stderr"])):
        fail("sealed retry teardown-race receipt differs")
    if package.get("post_exit_drain") != {"maximum_seconds": 30.0, "poll_seconds": 0.1}:
        fail("post-exit drain contract differs")
...
def validate_authorities(require_fresh: bool, require_low_vram: bool = True) -> tuple[dict, dict, dict, dict]:
    ...
    if (package.get("schema") !=
            "ninfer.r9700.bf16-gdn-control-t1-whole-ab-retry2-package.v1" or
            package.get("status") not in {"prepared_for_independent_review", "reviewed_ready"} or
            package.get("production_routing_authorized") is not False or
            package.get("exact_invocation") != {
                "preflight":
                    "bash profiles/bench/r9700-bf16-gdn-control-t1-whole-ab-retry2-20260920/commands.sh --preflight",
                "measure":
                    "bash profiles/bench/r9700-bf16-gdn-control-t1-whole-ab-retry2-20260920/commands.sh --measure"} or
            package.get("order") !=
            ["control", "candidate", "candidate", "control", "control", "candidate"]):
        fail("retry package disposition differs")
    validate_retry_failure(package)
...
def write_plan() -> None:
    ...
    if package["status"] != "reviewed_ready":
        fail("independent review required before creating a measurement plan")
    sources = [PACKAGE_PLAN, PACKAGE / "commands.sh", PACKAGE / "prepare.py", PACKAGE / "run.py"]
    plan = {"schema": "ninfer.r9700.bf16-gdn-control-t1-whole-ab-retry2-plan.v1",
            ...
            "retained_retry_failure": package["retained_retry_failure"],
            "imported_helpers": package["imported_helpers"],
            "post_exit_drain": package["post_exit_drain"],
            "measurement_limitations": package["measurement_limitations"],
    ...
    with os.fdopen(descriptor, "w") as stream:
        ...
        stream.write("\n")
    print("r9700_bf16_gdn_control_t1_whole_ab_retry2_prepare: PASS")
...
def main() -> int:
    ...
    if args.preflight:
        ...
        print("r9700_bf16_gdn_control_t1_whole_ab_retry2_preflight: PASS "
