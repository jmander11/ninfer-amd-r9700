...
private:
    ...
    MergerW merger_{};
    std::int32_t q4_workspace_columns_ = 0;
    std::int32_t w8_workspace_columns_ = 0;
};
...
public:
    VisionPrefillSession(DeviceContext& device, const LoadedModelData& model,
                         WorkspaceArena& workspace, qwen3::PreparedPromptData& prompt,
...
private:
    ...
    std::vector<DeviceEventTimer> timers_;
};
