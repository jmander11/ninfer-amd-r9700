...
VisionContext::VisionContext(DeviceContext& ctx, const LoadedModelData& weights) : ctx_(ctx) {
    ...
    const auto include_integer = [&](const Weight& weight) {
        if (weight.qtype == QType::Q4G64_F16S) {
            q4_workspace_columns_ = std::max(q4_workspace_columns_, weight.k);
        } else if (weight.qtype == QType::W8G32_F16S) {
            w8_workspace_columns_ = std::max(w8_workspace_columns_, weight.k);
        }
    };
    include_integer(*patch_embed_);
    for (const BlockW& block : blocks_) {
        include_integer(*block.qkv);
        include_integer(*block.projection);
        include_integer(*block.fc1);
        include_integer(*block.fc2);
    }
    include_integer(*merger_.fc1);
    include_integer(*merger_.fc2);
}

std::size_t VisionContext::workspace_bytes(const qwen3::VisionItemControl& item,
                                           WeightsProfile weights_profile) {
    const std::size_t layout_bytes =
        build_workspace_layout(item.patch_count, item.merged_count,
                               static_cast<std::size_t>(item.segment_count))
            .bytes;
    return checked_add(layout_bytes,
                       aligned_linear_reserve(Variant::vision_linear_workspace_capacity_bytes(
                           weights_profile, static_cast<std::int32_t>(item.patch_count))),
                       "workspace");
}
...
std::size_t VisionContext::workspace_capacity_bytes(std::uint32_t max_merged_tokens,
    ...
    const std::size_t patches =
        checked_mul(max_merged_tokens, VisionScheduleConfig::merge_unit, "capacity patch count");
    const std::size_t layout_bytes =
        build_workspace_layout(patches, max_merged_tokens, segments).bytes;
    return checked_add(layout_bytes,
                       aligned_linear_reserve(Variant::vision_linear_workspace_capacity_bytes(
                           weights_profile, static_cast<std::int32_t>(patches))),
                       "workspace capacity");
}
...
VisionPrefillSession::VisionPrefillSession(DeviceContext& device, const LoadedModelData& model,
                                           ...
                                           qwen3::PreparedPromptData& prompt,
...
VisionChunk VisionPrefillSession::prepare_chunk(std::uint32_t begin, std::uint32_t nominal_length) {
    ...
    const qwen3::VisionItemControl& control = plan_.control->items[active->item_index];
    const qwen3::VisionItem& source         = prompt_.vision_items[active->item_index];
...
double VisionPrefillSession::elapsed_seconds() const {
    ...
    for (const DeviceEventTimer& timer : timers_) { milliseconds += timer.elapsed_ms(); }
