...
// Typed Qwen3 phase-root allocation clusters shared by the real schedule and its startup
...
namespace ninfer::targets::qwen3::detail::NINFER_QWEN3_RUNTIME_NS::workspace_recipe {
...
TextAttentionResultRoots text_attention_results(Allocator& allocator, std::int32_t tokens) {
    return {
        matrix(allocator, DType::BF16, Config::query_size, tokens),
        matrix(allocator, DType::BF16, Config::kv_size, tokens),
        matrix(allocator, DType::FP32, Config::query_size, tokens),
        matrix(allocator, DType::BF16, Config::query_size, tokens),
    };
}
