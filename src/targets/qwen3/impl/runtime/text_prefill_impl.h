...
PrefillChunkResult prefill_text_chunk(
    ...
    if (state.mtp_cache != nullptr) {
        if (state.mtp_kv_allocation == nullptr || state.mtp_kv_publication == nullptr ||
            ...
            throw std::logic_error("MTP prefill has no FP8-K/INT4-V transaction authority");
        }
...
void sample_from_hidden(PrefillContext& state, const Tensor& hidden, std::int32_t absolute_position,
    ...
    if (state.execution.linear_execution != nullptr) {
        state.execution.linear_execution->linear(hidden, state.execution.model.output_head, logits,
                                                 state.execution.work,
                                                 state.execution.device.stream);
    } else {
        ops::linear(hidden, state.execution.model.output_head, logits, state.execution.work,
                    state.execution.device.stream);
    }
