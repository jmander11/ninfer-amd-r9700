...
RoundStateLayout begin_round_state_layout(LayoutBuilder& builder, const RoundStateSpec& spec) {
    ...
    layout.backend_kv_cursor =
        add_tensor(builder, DType::I32, {batch}, "backend KV segmented cursor");
    return layout;
}
...
void complete_round_state_layout(LayoutBuilder& builder, RoundStateLayout& layout) {
    ...
    if (layout.spec.enable_mtp) {
        ...
        decode.updated_frontiers =
            add_tensor(builder, DType::I32, {batch}, "MTP decode updated frontiers");
        decode.verify_ids =
...
MtpDecodeState::MtpDecodeState(DeviceSpan backing, const MtpDecodeStateLayout& layout,
    ...
    updated_frontiers = layout.updated_frontiers.bind(backing);
    licensed_tokens =
...
RoundState::RoundState(DeviceSpan backing, const RoundStateLayout& layout) {
    ...
    text_kv_status       = layout.text_kv_status.bind(backing);
    text_kv_cursor       = layout.text_kv_cursor.bind(backing);
    backend_kv_status    = layout.backend_kv_status.bind(backing);
    backend_kv_cursor    = layout.backend_kv_cursor.bind(backing);
