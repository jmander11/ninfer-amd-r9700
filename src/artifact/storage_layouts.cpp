...
std::string_view layout_name(StorageLayout layout) noexcept {
    ...
    case StorageLayout::RowScaledK128V1:
...
std::uint64_t tensor_encoded_size(StorageLayout layout, NumericFormat format,
    ...
    if (layout == StorageLayout::RowScaledK128V1) {
        if (shape.size() != 2 || shape[0] == 0 || shape[1] == 0) {
            throw ArtifactError("row-scaled-k128-v1 requires a positive rank-two shape");
        }
        return row_scaled_geometry(format, shape).encoded_bytes;
    }
    if (layout == StorageLayout::R9700Q4G64N16K16V1) {
        if (format != NumericFormat::Q4G64_F16S) {
...
RowScaledGeometry row_scaled_geometry(NumericFormat format,
    ...
    RowScaledGeometry out;
    out.rows             = shape[0];
    out.columns          = shape[1];
    out.padded_columns   = align_up(shape[1], kKAlignment, "padded K");
    out.code_plane_bytes = checked_mul(out.rows, out.padded_columns, "FP8 code plane bytes");
