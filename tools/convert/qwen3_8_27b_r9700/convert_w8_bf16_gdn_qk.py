"""Build the W8G32 plus BF16 GDN query/key evaluator."""
...
class W8Bf16GdnQueryKeyConversionPreflight:
    model_dir: Path
...
def preflight_conversion(
    ...
    object_plan = family_conversion.build_object_plan(
        w8_bf16_gdn_qk_inventory.OBJECT_SPECS, resource_map
    )
    ...
    return W8Bf16GdnQueryKeyConversionPreflight(
        model_dir=model,
...
def _materialize_tensor(
    spec: w8_bf16_gdn_qk_inventory.TensorSpec,
    reader: ShardReader,
    draft: draft_head.DraftHeadContext,
) -> torch.Tensor:
...
def _encode_tensor(
    ...
    if (
        spec.format not in w8_bf16_gdn_qk_inventory.DIRECT_FORMATS
    ...
    *,
    ...
    if output.exists():
        raise FileExistsError(
            f"refusing to overwrite existing W8/BF16-GDN-QK evaluation artifact: {output}"
        )
    ...
    report = family_conversion.build_conversion_report(
        ...
        target_key=w8_bf16_gdn_qk_inventory.TARGET_KEY,
        recipe_id=w8_bf16_gdn_qk_inventory.RECIPE_ID,
    ...
    report["candidate"] = {
        ...
        "reason": (
            "complete attributed GDN query/key family within the fixed promotion and "
            "32K-headroom budget"
        ),
    }
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    print(f"W8/BF16-GDN-QK evaluation conversion report: {report}")
