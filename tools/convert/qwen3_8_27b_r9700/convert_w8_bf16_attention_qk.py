"""Build the W8G32 plus BF16 full-attention query/key evaluator."""
...
class W8Bf16AttentionQueryKeyConversionPreflight:
    model_dir: Path
...
def preflight_conversion(
    ...
    object_plan = family_conversion.build_object_plan(
        w8_bf16_attention_qk_inventory.OBJECT_SPECS, resource_map
    )
    ...
    return W8Bf16AttentionQueryKeyConversionPreflight(
        model_dir=model,
...
def _materialize_tensor(
    spec: w8_bf16_attention_qk_inventory.TensorSpec,
    reader: ShardReader,
    draft: draft_head.DraftHeadContext,
) -> torch.Tensor:
...
def _encode_tensor(
    ...
    if (
        spec.format not in w8_bf16_attention_qk_inventory.DIRECT_FORMATS
    ...
    *,
    ...
    if output.exists():
        raise FileExistsError(
            f"refusing to overwrite existing W8/BF16-attention-QK evaluation artifact: {output}"
        )
    ...
    report = family_conversion.build_conversion_report(
        ...
        target_key=w8_bf16_attention_qk_inventory.TARGET_KEY,
        recipe_id=w8_bf16_attention_qk_inventory.RECIPE_ID,
    ...
    report["candidate"] = {
        ...
        "comparison_role": (
            "all-W8G32 except represented source-BF16 query/key projections "
            "in every full-attention layer"
        ),
        "tensor_encoded_bytes": w8_bf16_attention_qk_inventory.TENSOR_ENCODED_BYTES,
        "device_arena_bytes": w8_bf16_attention_qk_inventory.DEVICE_ARENA_BYTES,
        "reason": (
            "complete full-attention query/key family within the fixed promotion and "
            "32K-headroom budget"
        ),
    }
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    print(f"W8/BF16-attention-QK evaluation conversion report: {report}")
