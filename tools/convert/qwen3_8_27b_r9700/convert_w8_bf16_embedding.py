"""Build the registered all-W8G32 plus source-BF16 token-embedding evaluator."""
...
class W8Bf16EmbeddingConversionPreflight:
    model_dir: Path
...
def preflight_conversion(
    ...
    object_plan = family_conversion.build_object_plan(
        w8_bf16_embedding_inventory.OBJECT_SPECS, resource_map
    )
    ...
    return W8Bf16EmbeddingConversionPreflight(
        model_dir=model,
...
def _materialize_tensor(
    spec: w8_bf16_embedding_inventory.TensorSpec,
    reader: ShardReader,
    draft: draft_head.DraftHeadContext,
) -> torch.Tensor:
...
def _encode_tensor(
    ...
    if (
        spec.format not in w8_bf16_embedding_inventory.DIRECT_FORMATS
    ...
    *,
    ...
    if output.exists():
        raise FileExistsError(
            f"refusing to overwrite existing W8/BF16-embedding evaluation artifact: {output}"
        )
    ...
    with ShardReader(preflight.model_dir) as reader:
        with ArtifactWriter(
            ...
            for index, spec in enumerate(w8_bf16_embedding_inventory.OBJECT_SPECS, start=1):
                ...
                del payload
                print(
                    f"[{index}/{len(w8_bf16_embedding_inventory.OBJECT_SPECS)}] {spec.name}",
                    flush=True,
                )
    ...
    report = family_conversion.build_conversion_report(
        ...
        target_key=w8_bf16_embedding_inventory.TARGET_KEY,
        recipe_id=w8_bf16_embedding_inventory.RECIPE_ID,
    ...
    report["candidate"] = {
        ...
        "comparison_role": "all-W8G32 matrices with represented source-BF16 token embedding",
        "tensor_encoded_bytes": w8_bf16_embedding_inventory.TENSOR_ENCODED_BYTES,
        "device_arena_bytes": w8_bf16_embedding_inventory.DEVICE_ARENA_BYTES,
        "reason": (
            "the measured BF16 output-head evaluator did not improve all-W8; the remaining "
            "source-Q6 vocabulary role feeds every token through the full residual stack"
        ),
    }
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    print(f"W8/BF16-embedding evaluation conversion report: {report}")
