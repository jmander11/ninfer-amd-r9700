"""W8G32 plus BF16 full-attention query/key evaluation inventory.

The fused query/key projection in every one of the 16 full-attention layers
retains its represented source BF16 values. Every other persistent matrix
remains W8G32. This is the complete full-attention query/key semantic family,
never a selected layer subset.
"""
...
RECIPE_ID = "r9700-w8g32-bf16-full-attention-query-key-eval-v0"


def _is_promoted(spec: TensorSpec) -> bool:
    return any(
        spec.name == f"text/layers/{layer}/attention/query_key"
...
def validate_inventory() -> None:
    if len(TENSOR_SPECS) != 1118 or len(OBJECT_SPECS) != 1124:
        raise ValueError("W8/BF16-attention-QK evaluator is not Qwen3.8-27B complete")
    ...
    if DEVICE_ARENA_BYTES != 30_810_928_384:
        raise ValueError("W8/BF16-attention-QK aligned device arena byte total differs")
