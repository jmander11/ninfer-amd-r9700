"""Mechanical artifact-writing helpers for the Qwen3 converter.
...
def build_conversion_report(
    *,
    ...
    with ranking.open("rb") as source:
        ranking_sha256 = hashlib.file_digest(source, "sha256").hexdigest()

    artifact = Path(out_path)
    artifact_bytes = artifact.stat().st_size
    if artifact_bytes != final_bytes:
        raise ValueError(
            "completed artifact size changed before report publication: "
            f"expected {final_bytes}, got {artifact_bytes}"
        )
    ...
    return {
        ...
        "source": {
            ...
            "ranking_bytes": ranking.stat().st_size,
