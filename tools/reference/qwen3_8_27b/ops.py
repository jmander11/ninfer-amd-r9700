"""Tensor-only implementation-profile diagnostic operators.

The model path uses library kernels (PyTorch SDPA and FLA) rather than
project-owned Python GPU kernels. It is not an independent oracle for the
production W8, cache-codec, or model results. The slow recurrence remains a
private small-shape oracle for the FLA recurrence formula alone.
"""
