"""Qwen3.8-27B PyTorch diagnostic schedule over the R9700 candidate artifact."""
...
from tools.reference.qwen3.common.sampling import Sampler
from tools.reference.qwen3.common.tap import NullTap
...
class RefModel:
    """Fixed Text/MTP implementation-profile diagnostic for the 27B target."""
    ...
    def __init__(
        ...
        *,
        ...
        if self.device.type == "cuda" and not torch.cuda.is_available():
            raise RuntimeError(
                "an accelerator was requested but the active PyTorch has no ROCm device support"
            )
        if prefill_chunk <= 0:
    ...
    def prepare(self, capacity: int, *, compile_codec: bool | None = None) -> None:
        ...
        self.state = ModelState(self.device, capacity)
