"""Typed binding of a generic NInfer artifact to the Qwen3.8-27B R9700 candidate.
...
def _text_contract() -> tuple[_ExpectedTensor, ...]:
    ...
    for layer in range(64):
        ...
        if layer in FULL_ATTENTION_LAYERS:
            tensors.extend(
                (
                    _tensor(prefix + "attention/query_key", (7168, 5120), W8),
                    _tensor(prefix + "attention/gate_value", (7168, 5120), W8),
                    _tensor(prefix + "attention/query_norm", (256,), BF16),
                    _tensor(prefix + "attention/key_norm", (256,), BF16),
                    _tensor(prefix + "attention/output", (5120, 6144), W8),
                )
            )
        else:
            tensors.extend(
                (
                    ...
                    _tensor(prefix + "gdn/query_key", (4096, 5120), W8),
                    _tensor(prefix + "gdn/value_z", (12288, 5120), W8),
                    _tensor(prefix + "gdn/norm", (128,), BF16),
                    _tensor(prefix + "gdn/output", (5120, 6144), W8),
                )
            )
        tensors.extend(
            (
                _tensor(prefix + "post_attention_norm", (5120,), BF16),
                _tensor(prefix + "mlp/gate_up", (34816, 5120), W8),
                _tensor(prefix + "mlp/down", (5120, 17408), W8),
            )
        )
    tensors.extend(
        (
...
def _draft_contract() -> tuple[_ExpectedTensor, ...]:
    return (
        _tensor("text/draft_head", (131072, 5120), W8),
...
def _vision_contract() -> tuple[_ExpectedTensor, ...]:
    tensors: list[_ExpectedTensor] = [
        _tensor("vision/patch_embedding", (1152, 1536), W8),
    ...
    for layer in VISION_LAYERS:
        ...
        tensors.extend(
            (
                _tensor(prefix + "attention/qkv", (3456, 1152), W8),
                _tensor(prefix + "attention/qkv_bias", (3456,), BF16),
                _tensor(prefix + "attention/output", (1152, 1152), W8),
                _tensor(prefix + "attention/output_bias", (1152,), BF16),
                _tensor(prefix + "mlp/fc1", (4304, 1152), W8),
                _tensor(prefix + "mlp/fc1_bias", (4304,), BF16),
                _tensor(prefix + "mlp/fc2", (1152, 4304), W8),
...
def _contract_signature(
    obj: _ExpectedObject,
) -> tuple[str, tuple[int, ...], str, str] | tuple[str, str]:
    if isinstance(obj, _ExpectedTensor):
        return obj.name, obj.shape, obj.format, obj.layout
    return obj.name, obj.encoding


def _candidate_signature(
    obj: candidate_inventory.StoredObjectSpec,
) -> tuple[str, tuple[int, ...], str, str] | tuple[str, str]:
    if isinstance(obj, candidate_inventory.TensorSpec):
        return obj.name, obj.shape, obj.format, obj.layout
    return obj.name, obj.encoding


if tuple(map(_contract_signature, _OBJECT_CONTRACT)) != tuple(
    map(_candidate_signature, candidate_inventory.OBJECT_SPECS)
):
    raise RuntimeError(
        "Qwen3.8-27B reference binding differs from the exact R9700 candidate inventory"
    )
...
_SUPPORTED_INVENTORIES = {
    inventory.WEIGHTS_ID: inventory
    for inventory in (
        candidate_inventory,
        q4_inventory,
        q4_w8_mse_inventory,
...
def _validate_inventory(artifact: Artifact) -> None:
    inventory = _SUPPORTED_INVENTORIES.get(artifact.identity.weights_id)
    if inventory is None or artifact.identity.model_id != MODEL_ID:
        raise BindingError(
            f"artifact identity is {artifact.identity!r}; expected Qwen3.8 R9700 "
            f"candidate or terminal evaluation identity"
        )
    expected_objects = inventory.OBJECT_SPECS
    if len(expected_objects) != 1124:
        raise RuntimeError("Qwen3.8-27B selected inventory must contain 1124 objects")
    if len(artifact.objects) != len(expected_objects):
        raise BindingError(
            f"artifact has {len(artifact.objects)} objects; expected {len(expected_objects)}"
        )
    expected_by_name = {obj.name: obj for obj in expected_objects}
...
class ArtifactBinding:
    ...
    def __init__(self, artifact: Artifact, *, owns_artifact: bool = False):
        ...
        if len(self.tensors) != 1118 or len(self.row_views) != 390:
            raise RuntimeError("incomplete Qwen3.8-27B typed binding")
        if len(self.axis_views) != 48:
            raise RuntimeError("incomplete Qwen3.8-27B GDN axis binding")
...
class VisionArtifactBinding:
    """Validate a selected artifact while exposing only its Vision typed view."""
