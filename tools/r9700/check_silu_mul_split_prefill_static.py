#!/usr/bin/env python3
"""Fail-closed gfx1201 static gate for production split17408 2D SiLU-multiply."""
...
def check(path: Path) -> dict[str, int | str | bool]:
    ...
    if re.search(r"\b(?:v_mul_hi_[iu]32|s_(?:u|i)?div|v_cvt_f32_u32)\b", body):
        raise ValueError("challenger contains integer coordinate division lowering")
    if not re.search(r"^\s*v_exp_f32(?:_e32)?(?:\s|$)", body, re.MULTILINE):
        raise ValueError("challenger lacks native FP32 exponential")
    if re.findall(r"^\s*(?:global|buffer)_atomic_\S+", body, re.MULTILINE):
        raise ValueError("challenger must not contain atomics")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    print("production=true selected_route=split17408_2d rows_minimum=128")
    return 0
