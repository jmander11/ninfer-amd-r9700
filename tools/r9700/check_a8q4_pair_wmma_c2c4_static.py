...
def main() -> int:
    ...
    symbols = sorted(set(re.findall(
        r"^\s*\.globl\s+(\S*a8q4g64_pair_wmma_c2c4_kernel\S*)\s+; -- Begin function",
        text, re.MULTILINE)))
    require(len(symbols) == 1, f"expected one exact paired-WMMA kernel, found {len(symbols)}")
    symbol = symbols[0]
    body_match = re.search(
        rf"^\s*\.globl\s+{re.escape(symbol)}.*?^\s*\.size\s+{re.escape(symbol)},",
        text, re.MULTILINE | re.DOTALL)
    require(body_match is not None, "paired-WMMA kernel body is missing")
    body = body_match.group(0)
    require("v_wmma_i32_16x16x32_iu4" in body,
            "paired-WMMA kernel lacks native gfx1201 IU4 WMMA")
    require("scratch" not in body.lower(), "paired-WMMA body contains scratch traffic")
    descriptor_match = re.search(
        rf"\.amdhsa_kernel\s+{re.escape(symbol)}(.*?)\.end_amdhsa_kernel", text, re.DOTALL)
    require(descriptor_match is not None, "paired-WMMA descriptor is missing")
    ...
    require(len(metadata) == 1, "paired-WMMA metadata missing or duplicated")
    for key, value in {
        ...
        require(re.search(rf"{re.escape(key)}:\s+{value}\b", metadata[0]) is not None,
                f"paired-WMMA metadata differs for {key}")
    receipt = (f"a8q4_pair_wmma_c2c4_static: PASS symbol={symbol} "
