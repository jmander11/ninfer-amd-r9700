...
{SYMBOL}:
{load}{shuffle_instruction}{scalar_instruction}{barrier_protocol}{gain}{row_store}  .amdhsa_group_segment_fixed_size {lds}
...
class StaticCheckTest(unittest.TestCase):
    ...
    def test_accepts_exact_production_kernel(self):
        ...
        self.assertEqual(result["lds"], 80)
        self.assertEqual(result["maximum_workgroup"], 640)
        self.assertEqual(result["barrier_pairs"], 1)

    def test_rejects_missing_global_load(self):
        with self.assertRaisesRegex(ValueError, "16-byte input"):
            self.run_check(fixture(global_load=False))

    def test_rejects_missing_vector_store(self):
        with self.assertRaisesRegex(ValueError, "row-vector store"):
            self.run_check(fixture(store=False))
    ...
    def test_rejects_missing_barrier(self):
        protocol = """  ds_store_b32 v0, v1
  ds_load_b128 v[16:19], v1
"""
        with self.assertRaisesRegex(ValueError, "one store/signal"):
            self.run_check(fixture(barrier_protocol=protocol))

    def test_rejects_reordered_barrier(self):
        protocol = """  ds_store_b32 v0, v1
  s_barrier_wait -1
  s_barrier_signal -1
  global_inv scope:SCOPE_SE
