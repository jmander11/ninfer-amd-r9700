...
namespace target = ninfer::targets::qwen3_8_27b;

struct Options {
    std::filesystem::path artifact = "out/qwen3_8_27b.ninfer";
    int device                     = 0;
    int warmup                     = 2;
    int repetitions                = 10;
...
Options parse_options(int argc, char** argv) {
    ...
    for (int index = 1; index < argc; ++index) {
        ...
        } else if (argument == "--no-device-graph") {
            options.use_device_graph = false;
        } else if (argument == "-h" || argument == "--help") {
            print_usage(argc > 0 ? argv[0] : "ninfer_qwen3_8_27b_mtp_round_bench");
            std::exit(0);
        } else {
...
int run(const Options& options) {
    ...
    engine.use_device_graph          = options.use_device_graph;
    ...
    std::cout << "format,ninfer_qwen3_8_27b_mtp_round_bench_v1\n";
    ...
    std::cout << "proposal_head,"
              ...
              << '\n';
    std::cout << "device_graph," << (options.use_device_graph ? "true" : "false") << '\n';
...
int main(int argc, char** argv) {
    try {
        ...
        std::cerr << "ninfer_qwen3_8_27b_mtp_round_bench: " << error.what() << '\n';
        return 1;
    }
}
